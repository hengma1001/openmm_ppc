...
OPENMM_EXPORT_AMOEBA void OpenMM_AmoebaVdwForce_setParticleParameters(OpenMM_AmoebaVdwForce* target, int particleIndex, int parentIndex, double sigma, double epsilon, double reductionFactor) {
    reinterpret_cast<OpenMM::AmoebaVdwForce*>(target)->setParticleParameters(particleIndex, parentIndex, sigma, epsilon, reductionFactor);
}
OPENMM_EXPORT_AMOEBA void OpenMM_AmoebaVdwForce_getParticleParameters(const OpenMM_AmoebaVdwForce* target, int particleIndex, int* parentIndex, double* sigma, double* epsilon, double* reductionFactor) {
    reinterpret_cast<const OpenMM::AmoebaVdwForce*>(target)->getParticleParameters(particleIndex, *reinterpret_cast<int*>(parentIndex), *reinterpret_cast<double*>(sigma), *reinterpret_cast<double*>(epsilon), *reinterpret_cast<double*>(reductionFactor));
}
OPENMM_EXPORT_AMOEBA int OpenMM_AmoebaVdwForce_addParticle(OpenMM_AmoebaVdwForce* target, int parentIndex, double sigma, double epsilon, double reductionFactor) {
    int result = reinterpret_cast<OpenMM::AmoebaVdwForce*>(target)->addParticle(parentIndex, sigma, epsilon, reductionFactor);
    return result;
}
